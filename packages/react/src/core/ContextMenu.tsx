...
interface Props extends ComponentProps<'div'> {
  ...
  /** Whether to render new files/directories before or after the trigger element. */
...
export function ContextMenu({
  ...
  const [state, setState] = useState<'IDLE' | 'ADD_FILE' | 'ADD_DIR'>('IDLE');
  ...
  function onCloseAutoFocus(event: Event) {
    if ((state === 'ADD_FILE' || state === 'ADD_DIR') && inputRef.current) {
    ...
    <Root>
      ...
      <Portal>
        ...
        >
          ...
          <MenuItem icon="i-ph-folder-plus" onClick={() => setState('ADD_DIR')}>
